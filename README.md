...
The complete midterm video evidence package is available through the latest GitHub Release.
...
The FastAPI service starts and exposes the configured routes.
GET /health reports the database-connection state.
Student and advisor navigation routes load successfully.
Course data can be loaded from the configured database or bundled prototype data.
The generated course and competency workbooks contain the documented record counts and worksheets.
Screenshots and video evidence demonstrate the implemented workflows.
